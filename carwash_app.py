...
class CarwashApp:
    ...
    def create_login_screen(self):
        ...
        # Employee name
        ttk.Label(login_frame, text="Employee Name:", font=('Arial', 12)).pack(pady=5)
        self.name_entry = ttk.Entry(login_frame, font=('Arial', 12), width=30)
        self.name_entry.pack(pady=5)
        
        # Role selection
        ttk.Label(login_frame, text="Role:", font=('Arial', 12)).pack(pady=(20, 5))
        self.role_var = tk.StringVar(value="washer")
        
        role_frame = ttk.Frame(login_frame)
        role_frame.pack(pady=5)
        
        ttk.Radiobutton(role_frame, text="Washer", variable=self.role_var, 
                       value="washer", font=('Arial', 12)).pack(side='left', padx=20)
        ttk.Radiobutton(role_frame, text="Cashier", variable=self.role_var, 
                       value="cashier", font=('Arial', 12)).pack(side='left', padx=20)
        
        # Login button
        login_btn = ttk.Button(login_frame, text="Start Work", 
    ...
    def create_dashboard(self):
        ...
        ttk.Label(header_frame, text=f"Dashboard - {self.current_employee['name']} ({self.current_employee['role'].title()})", 
    ...
    def process_payment(self, car_id):
        ...
        # Car info
        ttk.Label(dialog, text=f"Car: {car_data['car_name']}", 
                 font=('Arial', 12, 'bold')).pack(pady=10)
        ttk.Label(dialog, text=f"Plate: {car_data['plate_number']}", 
                 font=('Arial', 10)).pack()
        
        # Payment amount
        ttk.Label(dialog, text="Payment Amount (₱):", font=('Arial', 12)).pack(pady=(20, 5))
        payment_entry = ttk.Entry(dialog, font=('Arial', 12), width=20)
    ...
    def show_add_car_dialog(self):
        ...
        # Car name
        ttk.Label(dialog, text="Car Name:", font=('Arial', 12)).pack(pady=5)
        car_name_entry = ttk.Entry(dialog, font=('Arial', 12), width=30)
        car_name_entry.pack(pady=5)
        car_name_entry.focus()
        
        # Plate number
        ttk.Label(dialog, text="Plate Number:", font=('Arial', 12)).pack(pady=5)
        plate_entry = ttk.Entry(dialog, font=('Arial', 12), width=30)
        plate_entry.pack(pady=5)
        
        # Photo section
        ttk.Label(dialog, text="License Plate Photo (optional):", font=('Arial', 12)).pack(pady=5)
